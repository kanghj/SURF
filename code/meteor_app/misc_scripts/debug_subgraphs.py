...
def guess_path_to_best_subgraph_file(API:str) -> Path:
    ...
    file_name = API + "_formatted_onlylabelled_" + experiment_id + str(request_counter) + "_result_best_subgraphs.txt"
...
def path_to_weighted_nodes_file(API:str) -> str:
    base_dir = project_path + "code/graphs/"
    file_name = API + "_formatted_onlylabelled_" + experiment_id + str(request_counter) + "_result_weighted_nodes.txt"
...
def debug2():
    ...
    while not os.path.exists( guess_path_to_best_subgraph_file (API_under_test)):
        request_counter -= 1
        # print("request_counter", request_counter)
        # print('could not find file. trying request_counter', request_counter)
        # print('path', guess_path_to_best_subgraph_file(API_under_test))
        if request_counter <= 0:
            break
        

    path = guess_path_to_best_subgraph_file(API_under_test)
    # print(path)
    feature_to_correct_count = read_features_file(API_under_test)
    ...
    with open(path, 'r') as f:
        for line in f:
            ...
            if line:
                splitted = line.split(',')
                best_subgraphs[int(splitted[0])] = float(splitted[1])

    sorted_best_subgraphs = sorted(best_subgraphs.items(), key=lambda x: x[1], reverse=True)
    sorted_best_subgraphs = [x[0] for x in sorted_best_subgraphs]
    ...
    convert(path_to_vert_map_file(API_under_test),
            ...
            path_to_weighted_nodes_file(API_under_test),
            sorted_best_subgraphs,
            feature_to_correct_count)


def convert(vertmap_path:str, edge_map_path:str, graph_path:str, weighted_node_path, only_show:list, feature_to_correct_count:dict):
    vertex_map = {}
    line_i = 0
    ...
    for item in only_show:
        ...
        for from_node in edges_from:
            ...
            for to_node in edges_from[from_node]:
                ...
                # print()
        print('+' if feature_to_correct_count[item] > 0 else '-')
        # for row in content[item].split('\n'):
        #     if row.startswith('v'):
        #         print(row.split(' ')[2] + (',+' if feature_to_correct_count[item] > 0 else ',-'))
            
    if len(only_show) == 0:
        print()
# run only if the caller indicated that we're not skipping
# print('run')
try:
    debug2()
